...
> Last updated: 2026-04-08 | Version: v4.5.10 — app live at systemaromania.site/flashcards
...
**Web hosting note:** cPanel (LiteSpeed) hosting requires TWO things for Vosk WASM to work:
...
```
2. `coi-serviceworker.js` in the `flashcards/` folder, loaded as the FIRST script in `<head>`:
```html
<script src="coi-serviceworker.js"></script>
```
**Vosk model path:** Must be absolute: `/flashcards/vosk-model-small-en-us-0.15.tar.gz`
The web server hosts the `.tar.gz` file (unlike Android APK which uses `.tar`).
